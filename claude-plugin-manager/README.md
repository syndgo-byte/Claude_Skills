...
- **중복 정리**: 같은 이름·비슷한 설명의 스킬이 여러 플러그인이나 claude.ai 동기화 스킬에 있으면 표시하고, 기능이 전부 겹치는 플러그인은 끌 수 있게 안내합니다.
- **무료 AI 혜택 (별도 아이콘)**: OpenRouter 무료 모델(종료일 포함), [Free-LLM 목록](https://github.com/nejib1/Free-LLM)의 상시 무료·충전형·체험 크레딧, Hacker News 소식을 6시간마다 수집합니다. 항목을 누르면 키 발급, 호출 예제, Cline·Aider·Claude Code 연결 방법이 담긴 안내가 열립니다. 새 혜택이 생기면 알림을 띄웁니다.
...
code --install-extension claude-plugin-manager-0.1.0.vsix
```
