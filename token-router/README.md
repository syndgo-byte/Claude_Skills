...
token-router는 사용자가 의식하지 않아도 **자동으로 라우팅을 개선합니다.**

**동작:**
1. 세션을 종료하면 `scheduler.js`가 자동 실행
2. 최근 50개 요청을 분석해서 라우팅이 정확한지 확인
3. 과도/저평가 패턴을 찾으면 `auto-updater.js`에 제안
4. 신뢰도 높은 패턴은 `route.js`에 자동 반영 예정

**현재 상태:**
- ✅ analyze-history.js: 분석 엔진 완성
- ✅ route.test.js: 13개 핵심 요청 테스트
- ⏳ auto-updater.js: 제안 생성 (수동 적용 중)
- ⏳ scheduler.js: SessionStart 훅 연결 대기

**사용자가 할 일:** 아무것도 없음. 평소처럼 대화하면 됩니다.
